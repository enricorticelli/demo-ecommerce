...
public sealed class ShippingCommandService(
    ...
    IViewMapper<Domain.Entities.Shipment, ShipmentView> mapper) : IShippingCommandService
{
    public async Task<ShipmentView> CreateAsync(CreateShipmentCommand command, CancellationToken cancellationToken)
    {
        ...
        var shipment = Domain.Entities.Shipment.Create(command.OrderId, command.UserId);
